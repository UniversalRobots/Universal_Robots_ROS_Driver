...
bool HardwareInterface::init(ros::NodeHandle& root_nh, ros::NodeHandle& robot_hw_nh)
{
  ...
  int servoj_gain = robot_hw_nh.param("servoj_gain", 800);
  ...
  double servoj_lookahead_time = robot_hw_nh.param("servoj_lookahead_time", 0.12);
...
void HardwareInterface::read(const ros::Time& time, const ros::Duration& period)
{
  ...
  if (data_pkg)
  {
    ...
    if(ros::Time::now() - last_publish_heartbeat_time > ros::Duration(1.0) || robot_mode_pub_->msg_.mode != robot_mode_ ||
